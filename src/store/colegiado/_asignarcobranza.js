...
export default {
  ...
  actions: {
    Agregar: async function (context, datos) {
      datos = {
        asignarcobranza: datos,
      };
      var respuesta = null;
      await global._axios_post("/asignarcobranza/colegiado/agregar", datos, (res, res2) => {
        respuesta = res2;
      });
      return respuesta;
    },
    ...
    ListarPostulantes: async function (context, parametros) {
      var query_string = "";
      if (parametros != null)
        query_string = global._json_to_query_string(parametros);

      const ruta = "asignarcobranza/colegiado/postulante?" + query_string;
      var respuesta = null;
