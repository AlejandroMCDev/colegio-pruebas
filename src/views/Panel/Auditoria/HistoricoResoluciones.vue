<template>
  <div class="card-header">
    <h3 class="card-title">Historico Resoluciones</h3>
  </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      HistoricoResoluciones: "_auditoria/HistoricoResoluciones",
    }),
    openurlArchivo() {
      if (this.listadoRegistro.length > 0) {
        let url = `${this.iprutaFiles}auditoria/resoluciones/historico/pdf?fecha_inicio=${this.filtros.fecha_inicio}&fecha_fin=${this.filtros.fecha_fin}&cqfll=${this.filtros.cqfp}&dni=${this.filtros.dni}&nombreApellido=${this.filtros.nombreApellido}`;
    ...
    download_excel: async function () {
      ...
      var res = await this.HistoricoResoluciones(historico);
    ...
    cargarRegistros: async function (pagina = 1) {
      ...
      let res = await this.HistoricoResoluciones(historico);
